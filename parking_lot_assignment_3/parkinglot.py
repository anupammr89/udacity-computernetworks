...
class ParkingLotTopo(Topo):
	...
	# If n > 1, create additional switches and clients and wire them up
	for i in range(2, n+1):
		sn_name = 's' + str(i)
		hn_name = 'h' + str(i)
		print("creating switch {} and host {}".format(sn_name, hn_name))
		# Add switch and host
		sn = self.addSwitch(sn_name)
		hn = self.addHost(hn_name, **hconfig)

		# Append switch and host names to the list
		switch_list.append(sn)
		host_list.append(hn)

		#Wire up
		prev_sn_name = 's' + str(i-1)
		self.addLink(prev_sn_name, sn_name, port1=downlink, port2=uplink, **lconfig)
		self.addLink(hn_name, sn_name, port1=0, port2=hostlink, **lconfig)
...
def run_parkinglot_expt(net, n):
    ...
    sender_list = []
    for hn in host_list:
	sender = net.getNodeByName(hn)
	sender.sendCmd('iperf -c %s -p %s -t %d -i 1 -yc > %s/iperf_%s.txt' % (recvr.IP(), 5001, seconds, args.dir, recvr))
	sender_list.append(sender)

    for sender_n in sender_list:
	sender_n.waitOutput()
